    ...
    vocabulary: {
        ...
        departments: {
            "Front Office (Whiteboard)": [
                "Computer", "Room key", "Scanner", "Credit card machine", "Calculator", "File Rack", "Medical Box", 
                "Fax", "Brochures", "Candy / Sweets", "Tissue Box", "CCTV", "Pen", "Cold towel", "Welcome Juice", 
                "Documentation", "Map", "Sanitizer", "Pencil", "Book", "Counter", "Paper", "Phone", "Safe Box", 
                "Chair", "Pictures", "Name Card / Business Card", "Furniture", "Aircon / Remote", "Dust bin", 
                "Tablet", "WiFi", "Charger", "File", "Extension cord", "Cooler / Bottle", "Correction Pen", 
                "NRC / Passport", "Mouse / Keyboard", "Car Pass", "Eraser", "Luggage", "Room / Information", 
                "Keycard machine", "Money", "Plastic bag", "Deco", "Wall plug", "Pen holder", "Button", 
                "Emergency light", "Sellable items", "Lost and Found", "Buffet ticket", "Ruler", "Note Pad", 
                "Trolley", "Scissor", "Stapler", "Air Freshener", "Carpets", "Payment device", "Seal", 
                "Tape", "Fire Extinguisher", "Clock", "Thanaka stone", "Puncher", "Laundry bag / items"
            ],
            "Restaurant & Bar (Whiteboard)": [
                "High ball", "Soup Spoon", "Rice bowl", "Chopstick", "Knife", "Spoon", "Flower vase", "Napkin", 
                "Chair cover", "Wine stand", "Plate", "Dual tray", "Side station", "Tong", "Saucer", "Ladle", 
                "Service plate", "Martini glass", "Margarita glass", "Rock glass", "Brandy Balloon", "Highball", 
                "Sherry copita", "Cruet set", "Food cover / Dome plate", "Crumb Scraper", "Place Mat", "Scoop", 
                "Chafing dish", "Bar blade", "Pilsner glass", "Cave Machine / Scan", "Sound box / Mic", "Stopper", 
                "Reserved Sign", "Number Stand", "Counter table", "Chair", "Tray", "Fork", "Trolley", "Knife", 
                "Table cloth", "Table runner", "Tray stand", "Wine decanter", "Strainer", "Plate / Bar spoon", 
                "Wine chiller", "Fridge", "Spatula", "POS", "Dinner plate", "BB plate", "Dessert plate", 
                "Baby chain", "Menu", "Tom Collins glass", "Podium", "Shot glass", "Coffee cup", "Coffee Saucer",
                "Tea spoon", "Hurricane glass", "Water goblet", "Champagne flute", "Butter knife", "Fish knife", 
                "Steak knife", "Soup saucer", "Soup bowl", "Apron", "Tissue", "Water Jar", "Note Pad", "Tablet", 
                "Stationary", "Voucher", "Wall plug", "Lamp / decor", "Gloves", "Irish coffee cup", "Beer Mug", 
                "Bottle opener", "Toothpick holder", "Ice Bucket", "Salad knife", "Pasta Plate", "Condiment & oil set", 
                "Fruit Plate", "Milk Jar", "Sugar Spoon", "Shaker", "Dust bin", "Electric kettle", "Dish wash machine", 
                "Red wine glass / white", "Cocktail deco", "Egg rack / egg stand", "Juice dispenser", "Dummy waiter", 
                "Lobster pick", "Cheese grinder", "Sachets", "Slip clothes", "Wet tissue", "Welcome towel", 
                "Fire extinguisher", "Order slip", "Picture / decor", "Screen", "Bar Stool", "Draft beer machine", 
                "Pourer", "Bar Spoon", "Coffee Machine", "Lemon Squeezer / Washrooms", "Measuring Cup", "Stage", 
                "Peddler duster", "Waiters clothes", "Gloves", "WiFi", "Ash tray", "Napkin", "Clock", "Air fresher", 
                "Blender", "Ice Machine", "Table balancer", "Jigger", "Shisha / Hookah", "Cutting board", 
                "Coaster / Drip mat", "Plate warmer", "Computer", "Bill holder", "Lighter", "Money", "Sanitizer", 
                "Wrapping paper / Foil", "Take away box / bag", "Butter dish / Side plate", "Lazy Susan", 
                "Chopstick holder", "Straw", "Mudler", "Zester"
            ]
        }
    },
    emotions: {
...
let studentProfile = { name: '', email: '', phone: '', friendReflection: '', friendReflectionSaved: false };
...
function startScenarioRecording() {
    ...
    navigator.mediaDevices.getUserMedia({ audio: true })
        .then(stream => {
            ...
            if (status) {
                status.textContent = 'Recording... speak clearly and cover the scenario details.';
            }
        ...
        .catch(err => {
            if (status) {
                status.textContent = 'Microphone access is required to record the scenario.';
            }
...
function switchPlaylistTab(tab, data) {
    ...
    if (songsBtn && friendBtn) {
        songsBtn.className = tab === 'songs'
            ? 'px-4 py-2 rounded-full bg-indigo-500 text-white text-[11px] font-bold shadow'
            : 'px-4 py-2 rounded-full bg-slate-900 text-slate-300 text-[11px] hover:bg-slate-700';
        friendBtn.className = tab === 'friend'
            ? 'px-4 py-2 rounded-full bg-indigo-500 text-white text-[11px] font-bold shadow'
            : 'px-4 py-2 rounded-full bg-slate-900 text-slate-300 text-[11px] hover:bg-slate-700';
    }
...
function switchModule(moduleKey) {
    // Prevent leaving scenario before passing the recorded challenge
    ...
    // Prevent leaving playlist before the friend reflection is submitted
    ...
    // Gracefully stop active webcams to conserve device resources
    stopWebcam();
    ...
    if (moduleKey === "founder") {
                ...
                <div class="space-y-3" id="inspection-nodes">
                    ${data.pillars.map(item => `
                        <div id="card-${item.id}" onclick="startGroomingCamera('${item.id}')" class="p-4 bg-slate-900/40 border border-slate-700/60 rounded-xl cursor-pointer hover:border-indigo-500/40 transition flex gap-4 items-start">
                            <div class="w-16 h-16 rounded-lg bg-slate-950 border border-slate-800 shrink-0 overflow-hidden flex items-center justify-center text-slate-600 text-xl" id="slot-${item.id}">🖼️</div>
                            <div>
                                <h3 class="text-xs font-bold text-white uppercase tracking-wide flex items-center gap-2">
                                    <span class="w-2 h-2 rounded-full bg-slate-600" id="dot-${item.id}"></span> ${item.area}
                                </h3>
        ...
        container.innerHTML = `
            ...
            <div class="p-5 bg-slate-900/40 border border-slate-700/70 rounded-2xl space-y-4">
                ...
                <audio id="scenario-audio-preview" controls class="w-full hidden mt-3 bg-slate-950 rounded-2xl"></audio>
                <div id="scenario-result" class="text-xs text-slate-400"></div>
        ...
        const activeSongsClass = playlistActiveSubtab === 'songs'
            ? 'px-4 py-2 rounded-full bg-indigo-500 text-white text-[11px] font-bold shadow'
            : 'px-4 py-2 rounded-full bg-slate-900 text-slate-300 text-[11px] hover:bg-slate-700';
        const activeFriendClass = playlistActiveSubtab === 'friend'
            ? 'px-4 py-2 rounded-full bg-indigo-500 text-white text-[11px] font-bold shadow'
            : 'px-4 py-2 rounded-full bg-slate-900 text-slate-300 text-[11px] hover:bg-slate-700';
...
function startGroomingCamera(targetId) {
    ...
    // Highlight active target selection
    ...
    document.getElementById(`card-${targetId}`).className = "p-4 bg-slate-900/40 border-2 border-indigo-500 rounded-xl cursor-pointer transition flex gap-4 items-start shadow-lg bg-slate-900/80";
    ...
    navigator.mediaDevices.getUserMedia({ video: { width: 640, height: 480 } })
        ...
        .catch(err => {
            ...
            placeholder.innerHTML = `❌ <p class="text-xs text-rose-400 font-bold mt-1">Camera access blocked.</p><p class="text-[9px] text-slate-500">Enable permissions.</p>`;
        });
}
...
function capturePhoto() {
    ...
    // Inject photo snapshot back into specific card container slots
    const targetSlot = document.getElementById(`slot-${currentTargetTargetId}`);
    targetSlot.innerHTML = `<img src="${dataUrl}" class="w-full h-full object-cover rounded-lg"/>`;
    
    // Set validation status checks
    const targetDot = document.getElementById(`dot-${currentTargetTargetId}`);
    targetDot.className = "w-2 h-2 rounded-full bg-emerald-400 shadow-sm shadow-emerald-400";

    stopWebcam();
    
    // Reset video interface controls
